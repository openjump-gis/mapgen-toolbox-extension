...
public class GeometryTransformer {
  ...
  protected Geometry transformMultiPoint(MultiPoint geom, Geometry parent) {
    List<Geometry> transGeomList = new ArrayList<>();
  ...
  protected Geometry transformPolygon(Polygon geom, Geometry parent) {
    ...
    LineString shell = transformLinearRing(geom.getExteriorRing(), geom);
    ...
    for (int i = 0; i < geom.getNumInteriorRing(); i++) {
      LineString hole = transformLinearRing(geom.getInteriorRingN(i), geom);
    ...
    if (isAllValidLinearRings)
      return factory.createPolygon((LinearRing) shell, holes.toArray(new LinearRing[0]));
    else {
